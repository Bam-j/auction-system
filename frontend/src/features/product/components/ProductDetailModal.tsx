...
const ProductDetailModal = ({open, handleOpen, product: initialProduct}: ProductDetailModalProps) => {
  ...
  }, [open, initialProduct, handleOpen]);
  ...
  const handleBid = async () => {
    if (!checkAuth() || !product || product.auctionId === undefined) {
      return;
    }
    ...
    try {
      await bidAuction({
        productId: product.id, // Assuming the API changed to take productId or auctionId
  ...
  const handlePurchase = async () => {
    if (!checkAuth() || !product || product.fixedSaleId === undefined) {
      return;
    }
                ...
                <div className='grid grid-cols-1 md:grid-cols-3 gap-6'>
                  ...
                  <div className='md:col-span-2 flex flex-col gap-4'>
                    ...
                    {isAuction && (
                        ...
                        {product.endedAt
                            ? new Date(product.endedAt).toLocaleString('ko-KR', {
                              year: 'numeric',
                              month: 'numeric',
                              day: 'numeric',
                              hour: '2-digit',
