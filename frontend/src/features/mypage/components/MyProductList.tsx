...
const MyProductList: React.FC = () => {
  ...
  const fetchMyProducts = async (params: Record<string, any> = {}) => {
    setIsLoading(true);
    try {
      ...
      if (location.state?.openProductId) {
        setSelectedProduct({id: location.state.openProductId} as Partial<Product> & { id: number });
        setOpenModal(true);
      }
    } catch (error) {
  ...
  const handleEndSale = async (productId: number) => {
